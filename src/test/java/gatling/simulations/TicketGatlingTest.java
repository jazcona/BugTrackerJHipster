...
public class TicketGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all tickets").get("/api/tickets").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticket")
                        .post("/api/tickets")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticket_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ticket").get("${new_ticket_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created ticket").delete("${new_ticket_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ticket entity").exec(scn);
